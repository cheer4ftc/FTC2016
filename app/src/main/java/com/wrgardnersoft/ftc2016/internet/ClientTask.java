    ...
    protected Void doInBackground(Void... params) {
        ...
        for (int i = 0; (i < urlSuffix.length) && (!serverOK); i++) {
            ...
            try {
                ...
                rowIndex = 0;
                while (!tableOK) {
                    Log.i("Checking Table", Integer.toString(tableIndex) + " " + Integer.toString(table.select("tr").size()));
                    table = document.select("table").get(tableIndex);
                    rows = table.select("tr");

                    for (rowIndex = 0; (rowIndex < rows.size()) &&
                        ((rows.get(rowIndex).select("th").isEmpty()) ||
                            (!rows.get(rowIndex).select("th").get(0).text().contentEquals("Rank"))); rowIndex++) {

                        try {
                            Log.i("testing row", rows.get(rowIndex).select("td").get(0).text());
                        } catch (Exception e) {
                        }
                    }
...
//              Log.i("R2", teamResult2[0]);
    ...
    protected void onPostExecute(Void result) {
        ...
        if (!serverOK) {
            CharSequence text = "Error loading new data.\nTry tapping refresh.";
